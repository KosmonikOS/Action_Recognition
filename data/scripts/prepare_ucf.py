...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the UCF preparation script.

    Returns:
        Parsed command-line arguments namespace.
    """
    parser = argparse.ArgumentParser(
        description="Prepare UCF dataset: scan videos under root, normalize labels, and write semicolon CSV.",
    )
    parser.add_argument(
        "--root",
        type=Path,
        required=True,
        help="Path to UCF dataset root (e.g., contains UCF-101). CSV paths are relative to this root.",
    )
    parser.add_argument(
        "--ext",
        "--extensions",
        dest="extensions",
        nargs="+",
        default=["avi", "mp4", "mov", "mkv", "webm", "mpg", "mpeg"],
...
def main() -> None:
    """Entry point: scan videos, normalize labels, and write the CSV."""
    args = parse_args()
